...
- **仪表盘**：KPI 总览（`/`，含趋势图/账单明细）、组件仪表盘（`/dashboard1`，模型/推理/训练/语言/工具/应用商店/客户等区块）
- **应用管理**（`/application-management`）、**已购应用**（`/applications`）、**客户**（`/customers`）、**消息中心**（`/messages`）
- **工作流画布**（`/workflow/:appId`）：组件⇄节点双向转换、Traits 面板族、GuidedTour、自定义快捷键、复制粘贴
- **API 文档**、**计划任务**、**命令面板**（⌘K）、**国际化**（中/英）
